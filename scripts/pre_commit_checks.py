...
def check(
        ...
        if has_changelog:
            ...
            if current_branch != "master":
                ...
                if "CHANGELOG.md" in diff_files_res.stdout:
                    has_changelog = True
                    console.print("[green]✔ CHANGELOG.md modification detected in branch commits.[/]")
            
            if not has_changelog:
                console.print("[yellow]⚠ WARNING: CHANGELOG.md is not modified or staging is clean. Please ensure you update CHANGELOG.md.[/]")
                all_passed = False
    ...
    try:
        ...
        try:
            ...
            if linter_res.returncode == 0:
                console.print("[green]✔ Code styling checks passed (system Ruff check).[/]")
            else:
                console.print("[red]❌ Code style check failed (system Ruff check issues):[/]")
                console.print(linter_res.stdout or linter_res.stderr)
                all_passed = False
        except FileNotFoundError:
            console.print("[yellow]⚠ Ruff is not installed/runnable. Skipping lint check.[/]")
    ...
    if test_dir.exists() and test_dir.is_dir():
        ...
        try:
            ...
            try:
                ...
                console.print("[yellow]⚠ pytest is not installed/runnable. Skipping test execution.[/]")
    else:
    ...
    if all_passed:
        ...
        console.print("[bold red]❌ PRE-COMMIT CHECKS FAILED. Please resolve the warnings/errors above.[/]")
        raise typer.Exit(code=1)
